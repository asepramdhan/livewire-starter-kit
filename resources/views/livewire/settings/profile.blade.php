...
<section class="w-full">
  ...
  <x-settings.layout :heading="__('Profil')" :subheading="__('Perbarui nama dan alamat e-mail Anda')">
        ...
        <div>
          <flux:text class="mt-4">
            ...
            <flux:link class="text-sm cursor-pointer" wire:click.prevent="resendVerificationNotification">
              {{ __('Klik di sini untuk mengirim ulang e-mail verifikasi.') }}
            </flux:link>
          </flux:text>

          @if (session('status') === 'verification-link-sent')
          <flux:text class="mt-2 font-medium !dark:text-green-400 !text-green-600">
            {{ __('Tautan verifikasi baru telah dikirim ke alamat e-mail Anda.') }}
          </flux:text>
          @endif
        </div>
        @endif
